...
fn main() {
  ...
  for c in &mut input { if c.is_ascii_digit() { *c -= b'0'; } }
  let mut board : Vec<_> = input.split_mut(|&c| c == b'\n')
  ...
  modifying the pointee. Answers:
  
  * Rust doesn't usually like internal mutability. &T means non-mutable,
    which applies to fields and elements of T too. That's why board above
    is mutable. Can be bypassed with Cell<T>.
  * I don't know how to cheaply convert from &mut[&mut[T]] to &[&[T]].
    Not sure if it's even doable (like C++ const**).
  * We don't have two mut refs. Their usage don't overlap, so we are good.

  Interestingly, incr_all() needs to explicitly specify nested `mut` keyword.
  There seems to be no way to call it otherwise.

  I still don't understand why row.iter_mut() is needed in reset_flashed().
  Can't we normally iterate over slices without a problem? We didn't have to write
  board.iter().
  */
  ...
  for _ in 0..100 {
    incr_all(&mut board);
    flash_count += flash_if_high(&mut board);
