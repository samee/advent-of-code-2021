...
fn last_slot(side_room: &[u8;2]) -> usize {
  if side_room[1] == b'.' { 1 } else { 0 }
}
  ...
  while let Some((Reverse(cost), board)) = q.pop() {
    ...
    for j in 0..4 {
      for i in 0..11 {
        ...
        if board.side_rooms[j][0]!=b'.' {
          if let Some(delta_cost) = can_move_to_hallway(&board, i, j, 0) {
            q.push((Reverse(cost+delta_cost),
                    move_to_hallway(&board, i, j, 0)));
          }
        }else if board.side_rooms[j][1]!=b'.' {
          if let Some(delta_cost) = can_move_to_hallway(&board, i, j, 1) {
            q.push((Reverse(cost+delta_cost),
                    move_to_hallway(&board, i, j, 1)));
          }
        }
      }
    }
...
fn read_board() -> Result<Board, Box<dyn std::error::Error>> {
  ...
  let mut board = Board{hallway: [0; 11], side_rooms: [[0;2];4]};
  board.hallway = input[1].as_bytes()[1..12].try_into()?;
  for i in 0..4 {
    for j in 0..2 {
      board.side_rooms[i][j] = input[2+j].as_bytes()[1+ROOM_X[i]];
    }
  }
  Ok(board)
}
...
fn print_board(board: &Board) {
  ...
  println!("###{}###",
           (0..4).map(|i| (board.side_rooms[i][0] as char).to_string())
                 ...
                 .join("#"));
  println!("  #{}#",
           (0..4).map(|i| (board.side_rooms[i][1] as char).to_string())
                 .collect::<Vec<_>>()
                 .join("#"));
  println!("  #########");
}
